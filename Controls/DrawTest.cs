...
namespace Story_Crafter.Controls {
    public class DrawTest : MonoGame.Forms.NET.Controls.MonoGameControl {
        ...
        public void UpdateScreenData(Screen screen) {
            ...
            shader.UpdateTileData(screen.Data);
            gradient = renderContext.LoadGradient(screen.Gradient);
        ...
        protected override void Draw() {
            ...
            //Editor.spriteBatch.Draw(gradient, Editor.SwapChainRenderTarget.Bounds, Editor.SwapChainRenderTarget.Bounds, Color.White);
            //Editor.spriteBatch.End();
            //DrawTileLayers(Editor.SwapChainRenderTarget);
        }
        ...
        static readonly BlendState blendState = new() {
        ...
        void DrawObjectLayers(RenderTarget2D target) {
            ...
            for (int layer = 4; layer < 8; layer++) {
                int i = 1000 + 500 * (layer - 4);
                for (int y = 0; y < 10; y++) {
                    position.Y = y * 24f;
                    for (int x = 0; x < 25; x++) {
                        byte index = screen.Data[i];
                        if (index > 0) {
                            byte bank = screen.Data[i + 250];
                            position.X = x * 24f;
                            Texture2D texture = renderContext.GetObjectTexture(bank, index);
        ...
        void DrawScreenTexture(RenderTarget2D target) {
            Editor.GraphicsDevice.SetRenderTarget(target);
            Editor.GraphicsDevice.Clear(Color.Transparent);
            Editor.spriteBatch.Begin(samplerState: SamplerState.PointWrap);
        ...
        protected override void OnMouseMove(MouseEventArgs e) {
            ...
            if (isMouseDown) {
                ...
                camera.SetPosition(
                    MathF.Round(initialCameraPos.X + deltaX),
                    MathF.Round(initialCameraPos.Y + deltaY));
            }
        }
