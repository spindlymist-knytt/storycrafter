...
namespace Story_Crafter.Knytt {
    public partial class Story {
        ...
        public IReadOnlyDictionary<Int2, Screen> Screens => screens;

        string path;
        Properties props;
        SaveData defaultSave;
        Dictionary<Int2, Screen> screens;
        ...
        public Screen this[int x, int y] => screens.GetValueOrDefault(new Int2(x, y));

        public Screen this[Int2 position] => screens.GetValueOrDefault(position);

        public Screen this[string key] {
            get {
                if (Int2.TryFromString(key, out Int2 position)) {
                    return screens.GetValueOrDefault(position);
                }
                else {
                    return null;
                }
        ...
        public void AddScreen(Screen screen) {
            if (screens.ContainsKey(screen.Position)) {
                throw new Exception($"A screen already exists at {screen.Position}.");
            }
            screens[screen.Position] = screen;
        }
        ...
        public void DeleteScreen(Int2 position) {
